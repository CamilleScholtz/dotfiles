...
last0=$(ps -p $(pgrep -f -o "urxvt -name notification") -o args | grep -o "[0-9]" | head -1)
last1=$(expr $last0 + 1)
last2=$(expr $last0 + 2)
last3=$(expr $last0 + 3)
last4=$(expr $last0 + 4)
...
if [[ $exist -eq 1 ]]; then
	wmctrl -x -r notification$last0 -e "0,-1,84,-1,-1"
elif [[ $exist -eq 2 ]]; then
	wmctrl -x -r notification$last0 -e "0,-1,134,-1,-1"
	wmctrl -x -r notification$last1 -e "0,-1,84,-1,-1"
elif [[ $exist -eq 3 ]]; then
	wmctrl -x -r notification$last0 -e "0,-1,184,-1,-1"
	wmctrl -x -r notification$last1 -e "0,-1,134,-1,-1"
	wmctrl -x -r notification$last2 -e "0,-1,84,-1,-1"
elif [[ $exist -eq 4 ]]; then
	pkill -f "urxvt -name notification$last0"

	wmctrl -x -r notification$last1 -e "0,-1,184,-1,-1"
	wmctrl -x -r notification$last2 -e "0,-1,134,-1,-1"
	wmctrl -x -r notification$last3 -e "0,-1,84,-1,-1"
